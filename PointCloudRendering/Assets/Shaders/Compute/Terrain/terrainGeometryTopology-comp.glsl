...
void main()
{
	...
	vec3 normal = normalize(texture(texBumpSampler, vertexData[globalID].textCoord).rgb * 2.0f - 1.0f);
	...
	vertexData[globalID].normal = vec3(mModel * vec4(normal.x, normal.z, normal.y, 0.0f));

	// For each tile => 2 triangles
	const uint triangleGlobalID = globalID * 2;
