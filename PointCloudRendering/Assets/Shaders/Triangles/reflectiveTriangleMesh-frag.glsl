...
uniform samplerCube texCubeMapSampler;
...
uniform float baseColorWeight;
uniform float reflectiveWeight;

subroutine vec3 mixingType(const vec3 baseColor, const vec3 environmentColor);
subroutine uniform mixingType mixingUniform;
...
subroutine(mixingType)
vec3 baseColor(const vec3 baseColor, const vec3 environmentColor)
{
	return baseColor + environmentColor * .00001f;
}

subroutine(mixingType)
vec3 mixColor(const vec3 baseColor, const vec3 environmentColor)
{
	return mix(environmentColor, baseColor, baseColorWeight);
}
...
void main()
{
	...
	const vec4 reflectionColor = texture(texCubeMapSampler, vec3(-reflectedVector.x, reflectedVector.y, reflectedVector.z));
	const vec4 refractionColor = texture(texCubeMapSampler, vec3(refractedVector.x, -refractedVector.y, refractedVector.z));
	const vec3 environmentColor = mix(refractionColor.rgb, reflectionColor.rgb, reflectiveWeight);
	const vec3 colorBase = lightUniform(fragKad.rgb, fragKs.rgb, fragNormal, shadowDiffuseFactor, shadowSpecFactor);

	fColor = vec4(mixingUniform(colorBase, environmentColor), fragKad.w);
}
