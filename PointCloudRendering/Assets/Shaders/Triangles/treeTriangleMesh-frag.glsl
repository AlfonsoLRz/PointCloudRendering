...
void main()
{
	...
	const vec4 healthyColor = vec4(1.0f, 0.9735f, 0.9338f, 1.0f);
	const vec4 dryColor = vec4(0.8676f, 0.8183f, 0.6124f, 1.0f);
	vec4 lerpResult = mix(dryColor, healthyColor, rand(position.xy));
	...
	fColor = vec4(reflectionColor, 1.0f);
}
