...
bool Loader::LoadFile(std::string Path)
{
	...
	while (std::getline(file, curline))
	{
		...
		if (algorithm::firstToken(curline) == "v")
		{
			...
			LoadedUniqueVertices.emplace_back(vpos.X, vpos.Y, vpos.Z);
		}
...
glm::vec3 vector3ToVec3(const Vector3& vector3)
{
	return glm::vec3(vector3.X, vector3.Y, vector3.Z);
}

std::vector<glm::vec3> veticesToVectorOfVec3(const std::vector<Vertex>& vertices)
{
	...
	{
		result.push_back(vector3ToVec3(value.Position));
	}
